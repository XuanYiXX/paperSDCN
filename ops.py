...
def cvt_map(pred, show=False):
    ...
    # gth = sio.loadmat(os.path.join(PATH, gth_test))
    # gth=np.array(gth['mask_test'])
    gth = read_data(variables.PATH,variables.train_label_name, 'mask_train', variables.data_type)
    # gth = read_data(variables.PATH,'label.mat','map1', variables.data_type)
    tiff.imsave('label.tif',gth)
    pred = np.argmax(pred, axis=1)
    # pred = np.asarray(pred, dtype=np.int8) + 1
    print(pred)
    index = np.load(os.path.join('file/', 'index.npy'))
    tiff.imsave('index.tif',index)
    ...
    if show:
        ...
        plt.imshow(gth)
        plt.show()
    tiff.imsave('pred.tif',pred)
    tiff.imsave('result.tif',pred_map)
